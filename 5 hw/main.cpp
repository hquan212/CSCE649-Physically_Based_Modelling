...
int gravity = 50;     //value of gravity
double h = 0.008;	//timestep
int rotateon;
		...
		for(int j=0;j<distance_from_planes.size();j++){
			if(distance_from_planes[j]*next_dist[j] <= 0){
				...
				for(int k=0;k<v.size();k++){
					v[k].x = v[k].x - (1+coff_rest)*e_vector[j].x;
					v[k].y = v[k].y - (1+coff_rest)*e_vector[j].y;
					v[k].z = v[k].z - (1+coff_rest)*e_vector[j].z;

					// v'(n+1) = -r*v(n)*e*e + (1-p)*v(n) - v*e*e
					v[k].vx = -coff_rest*v[k].vx*e_vector[j].x*e_vector[j].x + (1-coeff_friction)*v[k].vx - v[k].vx*e_vector[j].x*e_vector[j].x;
					v[k].vy = -coff_rest*v[k].vy*e_vector[j].y*e_vector[j].y + (1-coeff_friction)*v[k].vy - v[k].vy*e_vector[j].y*e_vector[j].y;
					v[k].vz = -coff_rest*v[k].vz*e_vector[j].z*e_vector[j].z + (1-coeff_friction)*v[k].vz - v[k].vz*e_vector[j].z*e_vector[j].z;
				}
			}
		}
